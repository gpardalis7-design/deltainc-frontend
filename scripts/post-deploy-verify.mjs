...
// Phase 4b: a category archive must serve its <h1> + body + CollectionPage/ItemList JSON-LD.
function runArchiveChecks(slug) {
  console.log(`\ncategory archive checks (/category/${slug})`);
  const res = curl(`/category/${encodeURIComponent(slug)}`, { followRedirects: true });
  const ok = record(
    `archive /category/${slug} returns 200 html`,
    res.status === 200 && /text\/html/i.test(res.contentType),
    { detail: res.error || `status ${res.status}, ${res.contentType || "no content-type"}` },
  );
  if (!ok || !res.body) return;
  ...
  const h1 = (body.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i) || [])[1]?.replace(/<[^>]*>/g, "").trim() || "";
  record("archive has a non-empty <h1>", h1.length > 0, { detail: h1 ? `“${h1.slice(0, 56)}”` : "none" });

  const hasBody = /class="archive-prerender"/.test(body);
  const textLen = body
    .replace(/<script[\s\S]*?<\/script>/gi, "")
    .replace(/<[^>]*>/g, " ")
    .replace(/\s+/g, " ")
    .trim().length;
  record("real archive body injected", hasBody && textLen > 100, {
    detail: `archive-prerender=${hasBody}, ~${textLen} chars visible text`,
  });
